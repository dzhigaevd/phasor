...
%                 axis vis3d;
        ...
        function plot_data_projections(postPhasor)
            try 
                postPhasor.data;
            catch
                error('There is no data available in the object!')
            end
            
            figure('Position',[100 100 2000 500]);
            subplot(1,3,1);imagesc(sum(postPhasor.data,3));
            axis image;title('Sum data [log], dimensions [1,2]');xlabel('Pixels');ylabel('Pixels');
            set(gca,'FontSize',20);
            
            subplot(1,3,2);imagesc(squeeze(sum(postPhasor.data,2)));
            axis image;title('Sum data [log], dimensions [1,3]');xlabel('Pixels');ylabel('Pixels');
            set(gca,'FontSize',20);
            
            subplot(1,3,3);imagesc(squeeze(sum(postPhasor.data,1)));
            axis image;title('Sum data [log], dimensions [2,3]');xlabel('Pixels');ylabel('Pixels');
            set(gca,'FontSize',20);
        ...
        function plot_displacement_slice(postPhasor, zoom_value) 
            try
                fprintf('Zoom values for the plots: [%.1f,%.1f,%.1f]\n',zoom_value(1),zoom_value(2),zoom_value(3));
            catch
                zoom_value = [1,1,1];   
                fprintf('Zoom values are set to default: [%.1f,%.1f,%.1f]\n',zoom_value(1),zoom_value(2),zoom_value(3));
            end
            ...
            set(gca,'FontSize',20);zoom(postPhasor.plotting.zoom_value(1));
            try
                caxis(postPhasor.plotting.displ_limits(1,:))
            catch
            end
            ...
            set(gca,'FontSize',20);zoom(postPhasor.plotting.zoom_value(2));
            try
                caxis(postPhasor.plotting.displ_limits(2,:))
            catch
            end
            ...
            set(gca,'FontSize',20);zoom(postPhasor.plotting.zoom_value(3));
            try
                caxis(postPhasor.plotting.displ_limits(3,:))
            catch
            end
        end
