...
%         userSpecFigName = true;
   ...
   %seg = seg/max(seg(:)); %Make 0-1. 
    ...
    jetmap = colormap('jet');
    bone = colormap('bone');
    the_cmaps = {graymap; multi; jetmap; bone};
...
%     else
        ...
        set(colormapPopup, 'String', {'gray'; 'multi-label'; 'jet'; 'bone'; 'user-spec'}, ...
        'Value', 4);
%     end
    ...
    xySupp = flipdim(xySupp, 1);
    
    xzview = flipdim(xzview, 1);
    xzSupp = flipdim(xzSupp, 1);
    
    xzview = permute(xzview, [3 2 1]);
    xzSupp = permute(xzSupp, [3 2 1]);
    %xzview = flipdim(xzview, 2);
    
    yzview = permute(yzview, [1 3 2]);
    yzSupp = permute(yzSupp, [1 3 2]);
    yzview = flipdim(yzview, 1);
    yzSupp = flipdim(yzSupp, 1);    
    ...
    imHandles{1} = imagesc(squeeze(xyview(:,:,sn(3))), segRange); set(imHandles{1},'AlphaData',single(squeeze(xySupp(:,:,sn(3))))); colormap(mycmap); colorbar; axis image; axis xy;
    ...
    imHandles{2} = imagesc(squeeze(yzview(sn(1),:,:)), segRange); set(imHandles{2},'AlphaData',single(squeeze(yzSupp(sn(1),:,:))));colormap(mycmap); colorbar; axis image; axis xy;
    ...
    imHandles{3} = imagesc(squeeze(xzview(:,sn(2),:)), segRange); set(imHandles{3},'AlphaData',single(squeeze(xzSupp(:,sn(2),:)))); colormap(mycmap); colorbar; axis image; axis xy;
    ...
    axis equal; axis vis3d; set(hslc(1:3),'LineStyle','none'); colorbar;
...
%         whichView = find(cell2mat(handles) == gca);
    ...
    function buttonUpCallback(varargin)
        ...
        axis equal; axis vis3d; colorbar; set(hslc(1:3),'LineStyle','none');
        xlabel 'p-a' ;ylabel 'l-r' ;zlabel 'i-s';
    ...
    function updateSliceViz(newslice)
        ...
        if whichView == 1
            ...
            set(imHandles{1},'AlphaData',single(squeeze(xySupp(:,:,sn(3)))));
        ...
        elseif whichView == 2
            ...
            set(imHandles{2},'AlphaData',single(squeeze(yzSupp(sn(1),:,:))));
            ...
            set(imHandles{3},'AlphaData',single(squeeze(xzSupp(:,sn(2),:))));
